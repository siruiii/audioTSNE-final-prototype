...
def save_chunk(chunk, index, start_time, output_dir, output_format):
    # Include both index and start_time in the output filename
    chunk.export(os.path.join(output_dir, f'{index + 1}_{start_time}.{output_format}'), format=output_format)
    ...
    if silence_based:
        ...
        with ThreadPoolExecutor() as executor:
            for i, chunk in enumerate(chunks):
                start_time = sum(len(chunks[j]) for j in range(i))  # Calculate the start time of the chunk
                executor.submit(save_chunk, chunk, i, start_time, output_dir, output_format).add_done_callback(lambda x: pbar.update(1))
        ...
        with ThreadPoolExecutor() as executor:
            ...
            if audio_length_ms % chunk_length_ms != 0:
                ...
                chunk = audio[start_time:end_time]
                executor.submit(save_chunk, chunk, num_chunks, start_time, output_dir, output_format).add_done_callback(lambda x: pbar.update(1))
